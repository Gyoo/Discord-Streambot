...
public class CCompactTest extends CommandTest{

    @BeforeMethod
    public void setUp() throws Exception{
        super.setUp();
        command = new CCompact(jda, dao);
    }
    ...
    public void testExecuteOn() throws Exception {
        ...
        HibernateUtil.getSession().refresh(guild);
        Assert.assertEquals(guild.isCompact(), true);
    }

    @Test
    public void testExecuteOff() throws Exception {
        this.initManager();
        GuildEntity guild = dao.getAll(GuildEntity.class).get(0);
        Assert.assertEquals(guild.isCompact(), false);
